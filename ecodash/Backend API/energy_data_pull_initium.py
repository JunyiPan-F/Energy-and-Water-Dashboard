...
eigs_build_dict = {
    "eigmeter-alpha-gamma-delta-bldg95_dept_lehigh_edu":"0095" ,
...
#Group the dataframe by computing the sum over it 
df = df.groupby('BUILDINGID', as_index=False).agg(Watts = ('Watts', 'sum'))
df['time'] = cpu_points[0]["time"]

# Retrieve the initium_layer table from its feature layer 
initium_layer = gis.content.get('2a0efee0b0d64aed9ca3df2166697802')
initium_table= initium_layer.tables[0]
...
initium_table.append(item_id = temp_item.id, upload_format = 'csv', source_info = temp_item_source_info, upsert = False)
