...
eigs_build_dict = { "eigmeter-alpha-gamma-delta-bldg95_dept_lehigh_edu":"0095" ,
    ...
    "eigmeter-hst-a_infra_lehigh_edu":"0205",
    "eigmeter-hst-b_infra_lehigh_edu":"0205",
    "eigmeter-hst-e_infra_lehigh_edu":"0205",
    "eigmeter-hst-f_infra_lehigh_edu":"0205",
    "eigmeter-hst-g_infra_lehigh_edu":"0205",
    "eigmeter-hst-h_infra_lehigh_edu":"0205",
    "eigmeter-hst-i_infra_lehigh_edu":"0205",
    "eigmeter-hst-j_infra_lehigh_edu":"0205",
    "eigmeter-preshouse-bldg28_dept_lehigh_edu":"0028"}
...
#query data 
query_one_day = 'SELECT host, time, wathtot FROM eig WHERE time > now() - 1440m '
query_two_day = 'SELECT host, time, wathtot FROM eig WHERE time < now() - 1440m and time > now() - 2880m '
...
print("eigmeters not reported between today and yesterday: ",list_difference_1, list_difference_2) 


building_metadata = []
for key,value in wath_reported_eigs.items():
    # appending [ BUILDINGID, WATHTODAY, TIME]
    building_metadata.append([eigs_build_dict[key],value, cpu_points_1[0]["time"]])


df = pd.DataFrame(building_metadata, columns = ['BUILDINGID', 'wathtot', 'time'])
df = df.groupby('BUILDINGID', as_index=False).agg(wathtot = ('wathtot', 'sum'))

#filter out the BIDs with missing eigmeters
for i in list_difference_1:
    print(i,"->",wath_reported_eigs[i])
    df = df.drop(df[df.BUILDINGID == eigs_build_dict[i]].index)
for i in list_difference_2:
    print(i)
    df = df.drop(df[df.BUILDINGID == eigs_build_dict[i]].index)

df['time'] = cpu_points_1[0]["time"]
print(df)


# Retrieve the  table from its feature layer 
# Initium ID : 4152aeae4eb3457cb1b8681a14c6f389

originis_layer = gis.content.get('96d100d260e9491eb06376ad1158aa06')
originis_table= originis_layer.tables[0]
originis_table


# Create a temporary CSV named according to current date and time
now = datetime.now().strftime('%Y%m%d%H%M%S')
temp_csv_name = f'temporary_{now}.csv'
...
# data = pd.read_csv(temp_csv_name)
# data

# Publish the csv layer in AGOL
...
temp_item_source_info = gis.content.analyze(item = temp_item.id)




originis_table.append(item_id = temp_item.id, upload_format = 'csv', source_info = temp_item_source_info, upsert = False)


# Delete the local temp csv file and the published csv feature layer 
