...
print('print out client: ');
print('%s', client); #know there's nothing wrong w/ client, see why influx db is not loaiding data 
...
quart_hours_per_year = 24*4*7*52
last_2_hours  = 8 
tot_df = pd.DataFrame()
for i in range(quart_hours_per_year):
    building_metadata_incr = []
    print("From : " + str(minute_before) + " to : " + str(minute_after))
    query = 'SELECT host, time, pavwat FROM eig WHERE time < now()-'+str(minute_before)+'m and time > now() - '+str(minute_after)+'m '
    ...
    for j in range(127):
        # print(cpu_points[j], "\n")
        if(eigs_build_dict.get(cpu_points[j]["host"]) != None):
            building_metadata_incr.append([ eigs_build_dict.get(cpu_points[j]["host"]), cpu_points[j]["time"], cpu_points[j]["pavwat"] ] )
        if(eigs_build_dict.get(cpu_points[j]["host"]) == None):
            print("ERROR : ", eigs_build_dict.get(cpu_points[j]["host"])," == ", cpu_points[j]["host"] )
            exit(0) 

    temp_df = pd.DataFrame(building_metadata_incr, columns = ['BUILDINGID', 'time', 'Watts'])
    temp_df = temp_df.groupby('BUILDINGID', as_index=False).agg(Watts = ('Watts', 'sum'))
    temp_df['time'] = cpu_points[0]["time"]
    # print(temp_df)
    tot_df = tot_df.append(temp_df, ignore_index=True)
...
tot_df.to_csv("AnnualDataWatts2Hr.csv")
